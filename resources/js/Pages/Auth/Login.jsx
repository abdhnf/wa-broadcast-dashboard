    ...
    <div className="min-h-screen bg-slate-50 dark:bg-[#0c0e12] text-slate-900 dark:text-slate-100 flex items-center justify-center p-4 font-sans selection:bg-emerald-500 selection:text-white">
      <div className="w-full max-w-sm space-y-4">
        ...
        <div className="text-center space-y-1">
          ...
          <p className="text-xs text-slate-500 dark:text-zinc-400">Masuk untuk mengelola blast dan gateway WhatsApp</p>
        </div>
        ...
        <div className="border border-slate-200 dark:border-zinc-800 rounded-2xl p-6 bg-white dark:bg-zinc-950 shadow-sm space-y-4">
          ...
          {googleAuthEnabled && (
            <div>
              <button
                ...
                className="w-full h-9 px-3 rounded-lg border border-slate-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 hover:bg-slate-50 dark:hover:bg-zinc-800 transition-colors flex items-center justify-center gap-2.5 text-xs text-slate-700 dark:text-zinc-200 font-medium cursor-pointer shadow-2xs"
              >
              ...
              <div className="relative my-3">
                ...
                <div className="relative flex justify-center text-[10px] uppercase font-mono">
                  <span className="bg-white dark:bg-zinc-950 px-2 text-slate-400 dark:text-zinc-500">atau dengan email</span>
                </div>
              </div>
            </div>
          )}
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            <div>
              <label className="text-[11px] font-medium text-slate-600 dark:text-zinc-400 block mb-1">Email</label>
              <input
                type="email"
                required
                value={email}
            ...
            <div>
              <label className="text-[11px] font-medium text-slate-600 dark:text-zinc-400 block mb-1">Password</label>
              <input
                type="password"
                required
                value={password}
        ...
        <div className="text-center text-[10px] text-slate-400 dark:text-zinc-500 font-mono">
          Laravel 13 &bull; Radix UI &bull; Fastify Gateway
        </div>
      </div>
    </div>
  );
